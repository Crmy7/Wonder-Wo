<template>
  <!-- Navigation Desktop -->
  <nav class="hidden md:block fixed top-0 left-0 right-0 z-50 border-b backdrop-blur-md bg-[var(--color-blanc)] border-[rgba(33,31,28,0.1)]">
      ...
      <div class="flex justify-between items-center h-16">
        ...
        <div class="flex items-center space-x-8">
          <NuxtLink
            ...
            :class="{ 'bg-beige text-[var(--color-primary)]': $route.path === '/' }"
          >
          ...
          <template v-if="authStore.isLoggedIn">
            <NuxtLink
              to="/profil"
              class="flex items-center space-x-2 px-3 py-2 rounded-lg text-[var(--color-grey-black)] hover:bg-beige transition-all duration-200"
              :class="{ 'bg-beige text-[var(--color-primary)]': $route.path === '/profil' }"
            >
            ...
            <NuxtLink
              to="/search"
              class="flex items-center space-x-2 px-3 py-2 rounded-lg text-[var(--color-grey-black)] hover:bg-beige transition-all duration-200"
              :class="{ 'bg-beige text-[var(--color-primary)]': $route.path === '/search' }"
            >
        ...
        <div class="flex items-center space-x-4">
          ...
          <div v-if="authStore.isLoggedIn && hasProfiles" class="relative">
            ...
            >
              <div class="w-8 h-8 bg-primary/20 rounded-full flex items-center justify-center">
                <span class="text-sm font-bold text-primary">{{ currentProfil?.nom?.charAt(0) || 'P' }}</span>
              </div>
              <span class="font-medium text-grey-black">{{ currentProfil?.nom || 'Sélectionner un profil' }}</span>
              <svg class="w-4 h-4 transition-transform" :class="{ 'rotate-180': isMenuOpen('profilSwitch') }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
              </svg>
            </button>

            <!-- Dropdown profils -->
            <div v-if="isMenuOpen('profilSwitch')" class="absolute top-full right-0 mt-1 w-64 bg-[var(--color-blanc)] rounded-xl shadow-lg border border-beige overflow-hidden animate-slideDown">
              <div class="p-3 border-b border-beige">
                <p class="text-sm font-medium text-grey-black">Changer de profil</p>
              </div>
              <div class="max-h-60 overflow-y-auto">
                ...
                >
                  ...
                  <div class="flex-1">
                    ...
                    <p class="text-xs text-grey-black/60">{{ profil.age }} ans</p>
                  </div>
              ...
              <div class="border-t border-beige">
                <NuxtLink to="/profil" @click="closeAllMenus" class="block px-3 py-3 text-sm accent-text hover:bg-beige transition-colors">
                  Gérer les profils
                </NuxtLink>
              </div>
            </div>
          ...
          <template v-else>
            <div class="relative">
              ...
              <!-- Menu utilisateur -->
              <div v-if="isMenuOpen('userMenu')" class="absolute top-full right-0 mt-1 w-48 bg-[var(--color-blanc)] rounded-xl shadow-lg border border-beige overflow-hidden animate-slideDown">
                <NuxtLink to="/profil" @click="closeAllMenus" class="block px-4 py-3 text-sm hover:bg-beige transition-colors">
                  <span class="accent-text">Mes profils</span>
                </NuxtLink>
                <div @click="closeAllMenus" class="block px-4 py-3 text-sm border-t border-beige hover:bg-beige transition-colors cursor-pointer">
                  <span class="accent-text">Paramètres</span>
                </div>
                <div @click="handleLogout" class="block px-4 py-3 text-sm border-t border-beige hover:bg-beige transition-colors cursor-pointer">
  ...
  <!-- Navigation Mobile -->
  <nav class="fixed bottom-0 left-0 right-0 z-50 border-t backdrop-blur-md bg-[var(--color-blanc)] border-[rgba(33,31,28,0.1)] md:hidden">
    <div class="flex justify-around items-center px-2 py-3 max-w-full">
      ...
      <template v-if="authStore.isLoggedIn">
        ...
        <NuxtLink
          to="/search"
          class="flex flex-col items-center justify-center min-w-0 flex-1 px-1 py-2 rounded-xl text-[var(--color-grey-black)] opacity-60 hover:bg-beige hover:opacity-80 transition-all duration-200"
          :class="{ 'bg-beige text-[var(--color-primary)] opacity-100': $route.path === '/search' }"
        >
          <div class="flex items-center justify-center mb-1 w-6 h-6 transition-transform"
               :class="{ 'scale-110': $route.path === '/search' }">
            <img src="/img/Search_WW.svg" alt="Recherche" class="w-6 h-6">
          </div>
          <span class="text-xs font-medium font-hashtag truncate">Recherche</span>
        </NuxtLink>
      </template>
      ...
      <template v-else>
        ...
        <NuxtLink
          to="/register"
          class="flex flex-col items-center justify-center min-w-0 flex-1 px-1 py-2 rounded-xl text-[var(--color-grey-black)] opacity-60 hover:bg-beige hover:opacity-80 transition-all duration-200"
          :class="{ 'bg-beige text-[var(--color-primary)] opacity-100': $route.path === '/register' }"
        >
          <div class="flex items-center justify-center mb-1 w-6 h-6 transition-transform"
          ...
          <span class="text-xs font-medium font-hashtag truncate">S'inscrire</span>
        </NuxtLink>
      </template>
    ...
    <div v-if="isMenuOpen('mobileMore')" class="fixed inset-0 z-40">
      <div class="absolute inset-0 bg-black bg-opacity-50" @click="closeAllMenus"></div>
      <div class="absolute bottom-20 right-4 bg-[var(--color-blanc)] rounded-xl shadow-lg overflow-hidden min-w-[160px] animate-slideUp">
        <!-- Switch profil mobile -->
        <template v-if="authStore.isLoggedIn && hasProfiles">
          <div class="px-4 py-3 border-b border-beige">
            <p class="text-xs font-medium text-grey-black mb-2">Profil actuel</p>
            <div class="flex items-center space-x-2">
              <div class="w-6 h-6 bg-primary/20 rounded-full flex items-center justify-center">
                <span class="text-xs font-bold text-primary">{{ currentProfil?.nom?.charAt(0) || 'P' }}</span>
              </div>
              <span class="text-sm font-medium">{{ currentProfil?.nom || 'Aucun profil' }}</span>
            </div>
          </div>
          
          <div @click="toggleMobileProfilSwitch" class="block px-4 py-3 text-sm border-b border-beige hover:bg-beige transition-colors cursor-pointer">
            <span class="accent-text">Changer de profil</span>
          </div>
          
          <!-- Liste profils mobile -->
          <div v-if="isMenuOpen('mobileProfilSwitch')" class="border-b border-beige">
            <div
              v-for="profil in profils"
              :key="profil.id"
              @click="selectProfil(profil)"
            ...
            >
              <div class="w-5 h-5 bg-primary/20 rounded-full flex items-center justify-center">
                <span class="text-xs font-bold text-primary">{{ profil.nom.charAt(0) }}</span>
              </div>
        ...
        <!-- Si non connecté -->
        <template v-if="!authStore.isLoggedIn">
          <NuxtLink to="/register" @click="closeAllMenus" class="block px-4 py-3 text-sm border-b border-gray-100 hover:bg-beige transition-colors cursor-pointer">
            <span class="accent-text">S'inscrire</span>
          </NuxtLink>
          <NuxtLink to="/login" @click="closeAllMenus" class="block px-4 py-3 text-sm border-b border-gray-100 hover:bg-beige transition-colors cursor-pointer">
        ...
        <!-- Si connecté -->
        <template v-else>
          <div @click="closeAllMenus" class="block px-4 py-3 text-sm border-b border-gray-100 hover:bg-beige transition-colors cursor-pointer">
            <span class="accent-text">Paramètres</span>
          </div>
          <div @click="handleLogout" class="block px-4 py-3 text-sm border-b border-gray-100 hover:bg-beige transition-colors cursor-pointer">
            <span class="accent-text text-secondary">Se déconnecter</span>
          </div>
        </template>

        <!-- Option commune -->
        <div @click="closeAllMenus" class="block px-4 py-3 text-sm hover:bg-beige transition-colors cursor-pointer">
